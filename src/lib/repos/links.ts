...
export class LinksRepo implements App.LinkRepoInterface {
  getAll = async () => {
    const { data } = await supabaseClient.from<App.DbLink>("shorten_links").select("id, link, shorten_link, copied");
  ...
  deleteLink = async (id: Pick<App.DbLink, "id">) => {
    const { data } = await supabaseClient.from<App.DbLink>("shorten_links").delete().match({ id });
